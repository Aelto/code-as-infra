...
The current industry standard is to use a "infrastructure as code" solution like Ansible for example where the tools are defined in sets of roles, combined & deployed via playbooks, and configured in huge inventories of YML config files. All of that is great but sometimes one of the core tools requires hundreds of line of configuration or uses a tech stack that is (in my opinion) not suited for production usage due to a lower general reliability like based python tools.
